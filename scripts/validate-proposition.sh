...
if echo "$lower_title" | grep -qiE '\b(is|are|was|were|has|have|had|does|do|did|can|could|should|would|will|shall|must|may|might|need|needs|make|makes|enable|enables|create|creates|cause|causes|lead|leads|require|requires|prevent|prevents|allow|allows|suggest|suggests|imply|implies|mean|means|prove|proves|show|shows|demonstrate|demonstrates|reduce|reduces|increase|increases|improve|improves|drive|drives|determine|determines|influence|influences|affect|affects|produce|produces|generate|generates|transform|transforms|emerge|emerges|become|becomes|depend|depends|connect|connects|extend|extends|contradict|contradicts|support|supports|challenge|challenges|explain|explains|predict|predicts|result|results|evolved|shifted|exceeded|exceeded|tripled|doubled|rose|fell|grew|shrank|faced|faces|built|remain|remains|remained|received|receive|outperform|outperforms|outperformed|project|projects|projected|defined|defines|separate|separates|separated|bootstrap|bootstraps|command|commands|addressed|address|maintained|maintains|ensured|ensures|combined|combines|scaled|scales|launched|launches|expanded|expands|achieved|achieves|introduced|introduces|adopted|adopted|aligned|aligns|applied|applies|assessed|assesses|classified|classifies|collected|collects|covered|covers|delivered|delivers|described|describes|designed|designs|developed|develops|established|establishes|estimated|estimates|evaluated|evaluates|evolved|evolved|exceeded|exceeded|focused|focuses|identified|identifies|included|includes|indicated|indicates|measured|measures|monitored|monitors|noted|notes|observed|observes|operated|operates|presented|presents|proposed|proposes|provided|provides|published|publishes|recommended|recommends|reported|reports|revealed|reveals|selected|selects|served|serves|specified|specifies|stated|states|targeted|targets|tested|tests|tracked|tracks|used|uses|validated|validates)\b'; then
    has_verb=true
fi
